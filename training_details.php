<?php
// MOCK DATA: Placeholder for "View Existing" scenario
$program = [
    "title" => "Morning Kata Mastery",
    "difficulty" => "Beginner",
    "start_time" => "06:00",
    "end_time" => "08:00",
    "venue" => "Main Dojo Hall",
    "description" => "Focus on basic forms and breathing techniques for yellow belts and up.",
    "prerequisites" => "White Belt w/ 2 Stripes minimum",
];
?>
...
<head>
    ...
    <title>Dojo-Sync | Program Details</title>
...
<body class="bg-brand-dark text-gray-800 font-sans h-screen flex overflow-hidden">
    ...
    <main class="flex-1 flex flex-col h-full relative bg-brand-dark">
        ...
        <div class="hidden md:flex justify-between items-center px-8 py-4 text-gray-400 text-sm bg-brand-dark shrink-0">
            <span>Training details | COACH</span>
            <div class="flex items-center gap-4">
        ...
        <div class="flex-1 bg-white md:rounded-tl-3xl overflow-y-auto p-4 md:p-8">
            <form action="save_training.php" method="POST" class="max-w-6xl mx-auto pb-10">

                <div class="flex justify-between items-start mb-6">
                    <h2 class="text-3xl font-extrabold text-black">
                        <?php echo $program['title'] ?: 'New Training Program'; ?>
                    </h2>
                    ...
                    <div class="flex gap-3 text-brand-blue text-xl shrink-0 ml-4">
                        ...
                        <button type="button" class="hover:text-red-700" onclick="alert('Are you sure you want to delete this program?')"><i class="fa-solid fa-trash"></i></button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                    ...
                    <div class="md:col-start-1 md:row-span-3">
                        <label class="block text-xs font-bold text-gray-500 mb-2 uppercase">Program Cover</label>
                    ...
                    <div class="md:col-start-2">
                        <label class="block text-xs font-bold text-gray-500 mb-2 uppercase">Difficulty Level</label>
                        <div class="relative">
                            <select name="difficulty" class="w-full bg-gray-200 rounded p-3 outline-none focus:ring-2 focus:ring-brand-yellow text-gray-700">
                                <option selected>Beginner</option>
                                <option>Intermediate</option>
                                <option>Advanced</option>
                                <option>Competition Elite</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="md:col-start-2">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="time" name="start_time" value="<?php echo $program['start_time']; ?>" class="w-full bg-gray-200 rounded p-3 outline-none focus:ring-2 focus:ring-brand-yellow text-gray-700">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-500 mb-2 uppercase">End Time</label>
                                <input type="time" name="end_time" value="<?php echo $program['end_time']; ?>" class="w-full bg-gray-200 rounded p-3 outline-none focus:ring-2 focus:ring-brand-yellow text-gray-700">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="md:col-start-2">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-gray-500 mb-2 uppercase">Venue</label>
                                <input type="text" name="venue" value="<?php echo $program['venue']; ?>" class="w-full bg-gray-200 rounded p-3 outline-none focus:ring-2 focus:ring-brand-yellow text-gray-700">
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-gray-500 mb-2 uppercase">Max Capacity</label>
                                <input type="number" name="capacity" value="20" class="w-full bg-gray-200 rounded p-3 outline-none focus:ring-2 focus:ring-brand-yellow text-gray-700">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="md:col-start-1">
                        ...
                        <textarea name="description" rows="4" class="w-full bg-gray-200 rounded-lg p-3 outline-none focus:ring-2 focus:ring-brand-yellow resize-none"><?php echo $program['description']; ?></textarea>
                    </div>

                    <div class="md:col-start-1">
                        <label class="block text-xs font-bold text-gray-500 mb-2 uppercase">Target Audience / Prerequisites</label>
                        <textarea name="prerequisites" rows="4" class="w-full bg-gray-200 rounded-lg p-3 outline-none focus:ring-2 focus:ring-brand-yellow resize-none"><?php echo $program['prerequisites']; ?></textarea>
                    </div>

                    <div class="md:col-start-2 flex items-end justify-end gap-4 mt-4">
                        <a href="training_management.php" class="flex-1 md:flex-none px-8 py-3 bg-gray-300 text-gray-700 font-bold rounded-lg hover:bg-gray-400 transition text-center">Cancel</a>
                        <button type="submit" class="flex-1 md:flex-none px-8 py-3 bg-brand-yellow text-brand-dark font-bold rounded-lg shadow-md hover:bg-yellow-400 transition">Save Program</button>
                    </div>
    ...
    <div id="mobileSidebar" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden" onclick="toggleSidebar()">
        <div class="w-64 h-full bg-brand-dark p-4 pt-10 relative" onclick="event.stopPropagation()">
            ...
            <nav class="space-y-2">
                <a href="training_management.php" class="block px-4 py-3 text-gray-400">Back to List</a>
                <a href="#" class="block px-4 py-3 bg-brand-yellow text-brand-dark font-bold rounded-lg">Program Details</a>
            </nav>
        </div>
    </div>
